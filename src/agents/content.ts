...
{"angle":"<one line: how today's research feeds the mandatory topic>",
 ...
 "citedSourceUrls":["<only URLs you actually drew on>"]}`;
  ...
  const out = await geminiJSON<Draft>(prompt, 0.65);
  // Replace the CTA token with the real link (the model sometimes echoes it
  // verbatim from guidance text), THEN enforce the X length limit.
  const xPost = enforceXLimit(applyCTAUrl(String(out.xPost ?? '').trim()));
  const linkedinPost = applyCTAUrl(String(out.linkedinPost ?? '').trim());
...
async function main() {
  ...
  // 3. Draft. Never leave the human with silence on failure.
  let degraded = false;
